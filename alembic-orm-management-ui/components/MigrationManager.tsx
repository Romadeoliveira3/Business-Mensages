              ...
              <li key={migration.id} className="p-4 sm:p-6 hover:bg-border-color/30 transition-colors duration-200">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2 mt-4 sm:mt-0">
                     <button onClick={() => downgrade(migration.down_revision || 'base')} disabled={!migration.down_revision} className="text-sm bg-gold-dark hover:bg-gold text-white font-semibold px-3 py-1.5 rounded-md transition disabled:opacity-50 disabled:cursor-not-allowed">
                        {t('migrationManager.downgrade')}
                      </button>
                     <button onClick={() => upgrade(migration.revision)} disabled={isCurrent} className="text-sm bg-accent-green hover:opacity-90 text-primary-dark font-bold px-3 py-1.5 rounded-md transition disabled:opacity-50 disabled:cursor-not-allowed">
