...
const App: React.FC = () => {
  ...
  // Carregar e salvar projetos no localStorage
  useEffect(() => {
    try {
  ...
  // Carregar dados quando o projeto ativo mudar
  useEffect(() => {
    if (activeProject) {
      console.log(`Fetching data for project: ${activeProject.projectName}`);
  ...
  const handleSaveProject = (projectData: Omit<AppConfig, 'id'> & { id?: string }) => {
    if (projectData.id) { // Editando projeto existente
      setProjects(projects.map(p => p.id === projectData.id ? { ...p, ...projectData } : p));
    } else { // Adicionando novo projeto
      const newProject: AppConfig = { ...projectData, id: `proj_${Date.now()}` };
      setProjects([...projects, newProject]);
      setActiveProjectId(newProject.id); // Ativa o novo projeto automaticamente
    }
